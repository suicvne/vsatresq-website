        ...
        selfserve.auth.checkLoginCookie((result) => {
            if (result) {
                ...
                { 
                    ...
                    else selfserve.auth.onLogin(result); 
                }
            }
            else
            {
                if(selfserve.auth.onLogin.constructor === Array) // Multiple on login functions
                    ...
                    else selfserve.auth.onLogin(undefined); 
            }
        });
    }
    else
...
const setCurrentUserObject = (ob) => {
    ...
    if (ob !== undefined) {
        ...
        if(typeof ob === 'string') selfserve.global.currentUser = JSON.parse(ob);
        else selfserve.global.currentUser = ob;
    }
}
 ...
 */
selfserve.auth.checkLoginCookie = function checkLoginCookie(callback) { // TODO: this function can be fixed big time.
    var _username = selfserve.auth.getCookie('username');
    ...
    if ($.trim(_username) && $.trim(_token)) {
        $.post("/blog/tokencheck", { username: _username, token: _token }, function (result, status, xhr) {
            ...
            if (callback) callback(selfserve.global.loggedIn);
        }).fail(() => {
    ...
    if ($.trim(_username) && $.trim(_password)) {
        $.post("./blog/loginnode", { username: _username, password: _password }, function (result, status, xhr) {
            ...
            if (status == "success") {
                ...
                if (callback) callback({status: status, result: result});
            }
            else // Err
            {
                if (callback) callback({ err: result, status: status });
            }
        }).fail((jqXHR, textStatus, errorThrown) => {
            if (callback) callback({ err: jqXHR.responseText, status: textStatus });
        });
    }
};
...
selfserve.auth.registerAccount = function registerAccount(_email, _username, _password, callback) {
    if ($.trim(_username) && $.trim(_email) && $.trim(_password)) {
        $.post("./blog/registernode", { username: _username, password: _password, email: _email },
            (result, status, xhr) => {
                if (status === "success") console.log('registration successful.');
